...
describe('Testes de carga e integração para userServices', () => {
  ...
  // Teste de carga para cadastrarUsuario com 2.000 execuções
  test('🧪 Executa 2.000 cadastros de usuários em paralelo', async () => {
    const numExecucoes = 2000; // reduzido de 10000 para 2000
    ...
    console.log(`Iniciando ${numExecucoes} cadastros de usuários...`);
    const start = Date.now();
    const resultados = await Promise.all(emails.map(email =>
  ...
  // Teste de carga para loginUsuario com 2.000 execuções
  test('🔐 Executa 2.000 logins simultâneos com usuários válidos', async () => {
    const numLogins = 2000; // reduzido de 10000 para 2000
    ...
    console.log(`Iniciando ${numLogins} logins simultâneos...`);
    const start = Date.now();
